                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST"){
                    ...
                    if ($dbconnect){
                        if (isset($_POST["Email"])){
                            ...
                            $aloginquery = "SELECT Admin_id FROM admins
                                WHERE Admin_username = '$login'";
                            $aloginresult = $dbconnect->query($aloginquery);
                            $aidfound = $aloginresult->fetch(PDO::FETCH_ASSOC);
                            if ($idfound == null && $aidfound == null){
                        ...
                        if ($idfound != null){
                            ...
                            if (isset($_POST["Password"])){
                                ...
                                else
                                {
                                    ...
                                    $_SESSION["Weight"] = $passfound["Weight"];
                                    $_SESSION["amountowe"] = $passfound["amountowe"];
                                    header("Location: profile.php");
                                    exit();
                                }
                            }
                        }elseif ($aidfound != null) {
							$aidfound = $aidfound["Admin_id"];
                            if (isset($_POST["Password"])){
                                $passquery = "SELECT * FROM admins
                                    WHERE Admin_id = '$aidfound';";
                                $passresult = $dbconnect->query($passquery);
                                $passfound = $passresult->fetch(PDO::FETCH_ASSOC);
                                if ($_POST["Password"] != $passfound["pass"])
                                    echo "password incorrect";
                                else
                                {
                                    ...
                                    $_SESSION["admin_id"] = $passfound["Admin_id"];
